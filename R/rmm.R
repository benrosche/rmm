...
level1adat <- data.frame(l1id=seq(1:10), l3id=1, x=rnorm(11, 0, 1))
...
build_data <- function(level1dat=NA, level2dat=NA, level3dat=NA) {
  ...
  level2 <<- level2dat %>%
    ...
    mutate(i2=cumsum(nl1), i1=lag(i2)+1) %>%
    mutate(i1 = ifelse(row_number()==1, 1, i1)) %>%
    dplyr::select(l1id, l2id, l3id, nl1, i1, i2, x)
...
build_model <- function(level12=NA, level23=NA, outcome_level=2, outcome_fun="Weibull") {
  if(level12=="MM") {
